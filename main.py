...
def show_info(text, subheading):
    ...
    tk.Label(info_frame, text=subheading, font=("Arial", 18), bg="#303236", fg="lightgreen").pack(pady=5)
...
def all_available_book_list_page():
                     for widget in main_f.winfo_children():
                                          widget.destroy()
                     show_info(lib.available_books(), "Available books")
...
def unret_books_page():
    ...
    if a == "":
            show_info("There are no books to be returned!", "Unreturned Books")
    else:
            show_info(a, "Unreturned Books")
